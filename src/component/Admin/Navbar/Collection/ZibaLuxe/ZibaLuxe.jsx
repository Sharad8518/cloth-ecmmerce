...
import styles from "./ZibaLuxe.module.css";
import { getProducts } from "../../../../api/user/Productapi";
import Pagination from "react-bootstrap/Pagination";
import {
  addCollectionToProducts,
  getAllLuxe,
  addOrUpdateLuxeobel,
  updateLuxeProducts,
  updateLuxeItem,
...
export default function ZibaLuxe() {
  ...
  const handleSubmit = async () => {
    try {
      const res = await addOrUpdateLuxeobel({
        title,
        description,
        files: files.map((f) => f.file), // pass only the File objects
        products: [], // optional: add selected product IDs if needed
      });

      alert("Luxeobel created!");
      console.log(res);
      ...
      console.error(err);
      alert("Error creating Luxeobel");
    }
  };

  const handleAddProduct = async (id) => {
    await updateLuxeProducts(id, { addProducts: ["productId1"] });
    fetchItems();
  };
  ...
  const handleDeleteImage = (index) => {
    ...
    if (!img.isNew) setRemovedImages([...removedImages, img.url]); // mark for deletion in backend
  ...
  const handleSave = async (id) => {
    ...
    removedImages.forEach((url) => data.append("removeImages[]", url));
    formData.images
    ...
    data.append(
      "existingImages",
      JSON.stringify(formData.images.filter((i) => !i.isNew))
    );

    await updateLuxeItem(id, data);
    setEditingId(null);
    fetchItems();
  };
   ...
   const handleAddProducts = async () => {
    ...
    try {
      ...
      await updateLuxeProducts(items[0]._id, {addProducts:selectedProducts });
      setSelectedProducts([]);
  ...
  const handleRemoveProducts = async () => {
    ...
    try {
      ...
      await updateLuxeProducts(items[0]._id, { removeProducts: selectRemoveProduct });
    ...
    <Container fluid className="p-4">
      ...
      <div>
        {items.map((item) => (
          <Row
            key={item._id}
            className="p-3 mb-4 border rounded shadow-sm align-items-center"
          >
            <Col md={10}>
              {editingId === item._id ? (
                <>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          {selectedProducts?.length > 0 && (
            <Button variant="primary" onClick={handleAddProducts} 
             disabled={addProductLoading || selectedProducts.length === 0}
            >
      ...
      <Modal show={show} onHide={handleClose} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title>Add Luxeobel</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            {/* Title */}
            ...
            {/* Drag & Drop area */}
            <div
              className="p-4 mb-3 border border-secondary rounded text-center"
              onDrop={handleDrop}
              onDragOver={handleDragOver}
              style={{ cursor: "pointer" }}
              onClick={() => document.getElementById("fileInput").click()}
            >
              <p className="mb-0">Drag & Drop files here or click to select</p>
              <input
                id="fileInput"
                type="file"
                multiple
