...
import { Container, Table, Badge, Button, Spinner, Pagination, Modal } from "react-bootstrap";
import { getProducts } from "../../../api/admin/productApi";
import { useNavigate } from "react-router-dom";
import SaleModal from "../../Navbar/SaleModal/SaleModal";

const ProductRow = ({ product, index, page, limit, onEdit, onDelete, onViewMore }) => {
...
const [showSaleModal, setShowSaleModal] = useState(false);
  return (
    ...
    <tr>
      ...
      <td>
        <Badge style={{ padding: 10 }} bg={product.status === "ACTIVE" ? "success" : "warning"}>
          {product.status || "-"}
        </Badge>
      </td>
      <td>
        <Badge style={{ padding: 10 }} bg={product.inStock ? "success" : "secondary"}>
      ...
      <td className="d-flex flex-wrap gap-1">
        ...
        <Button variant="secondary" size="sm" onClick={() => navigate("/dashboard/FrequentlyBought")}>
          Frequently Bought
        </Button>
        <Button variant="warning" size="sm" onClick={() => navigate("/dashboard/SimilarProduct")}>Similar Products</Button>
        <Button variant="dark" size="sm"   onClick={() => {
    setSaleProduct(product);
    setShowSaleModal(true);
  }}>Sale on Products</Button>
        <Button variant="primary" size="sm" onClick={() => onViewMore(product)}>
          View More
        </Button>
      </td>
    </tr>
  ...
  return (
    <Container className="my-5">
      ...
      {loading ? (
        ...
        <>
          <Table striped bordered hover responsive className="shadow-sm" style={{ fontSize: 14 }}>
            <thead>
              <tr>
                <th>#</th>
                <th>Item Number</th>
